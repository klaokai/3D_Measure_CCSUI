...
short GT_ADD_Buffer_continue(double AxisX_1, double AxisY_1,double Vel)//添加缓冲区指令
{
	short rtn;
...
short GT_ADD_Buffer_trigger(double AxisX_1, double AxisY_1,double Vel)//添加缓冲区指令
{
	short rtn;
...
short GT_GetLmtZero_X()//极限位置为坐标原点
{
	short axis_num = 1;
	short rtn;
	unsigned short status;
...
short GT_GetLmtZero_Y()//极限位置为坐标原点
{
	short rtn;
	short axis_num = 2;
	unsigned short status;
...
short GT_GetStartZero()//回到上电时的坐标(一定要是在调用GT_GetLmtZero函数建立系统极限坐标原点且回到该原点后方可调用该函数回到上电时的原点)
{
	short rtn;
...
short GT_GetZero()//回零，回到程序刚开始的零点(系统零点为刚上电时的坐标)
{
