...
/*!
	...
	\details
		\if FR
			...
			\Erreurs spécifiques : MCHR_ERROR_DLL_NOT_ACTIVE ( La DLL n'a pas ét?initialisée.	
		\else
...
/*!
	...
	\if FR
		\brief Obsolete! Fonction permettant d'ouvrir une connection ?un CHR sur le système.
	\else
		\brief Outdated! Function that opens a connection to a CHR.
	\endif
	\details
		\if FR
			\param [in] LPCSTR SensorName : Nom du coffret ?connecter.
			\param [in] enChrType SensorType : Type du coffret.
			\param [in] WORD IdSerialPort : Port serie ?utiliser pour la connexion.
			\param [in] DWORD BaudRate : BAUDRATE de la connexion.
			\param [in] LPCSTR ConfigFile : Fichier de configuration ?charger.
			\return short : un code d'erreur MCHR_ERROR
		\else
...
/*!
	...
	\details
		\if FR
			...
			\arg \c MCHR_ERROR_NOT_CONNECTED: pas de capteur trouv??l'adresse IP spécifiée. 
			\arg \c MCHR_ERROR_ADD_SENSOR: Une erreur s'est produite pendant l'ajout du capteur ?la liste.
			\arg \c MCHR_ERROR_NAME_ALREADY_EXISTS: un capteur du même nom existe déja dans la liste.
			\arg \c MCHR_ERROR_IP_ADDRESS: impossible de se connecter ?l'adresse spécifiée.
		\else
			\param [in] LPCSTR SensorName : Name of the housing to connect to.
			\param [in] enChrType SensorType : Type of the housing.
...
/*!
	...
	\details
		\if FR
			...
			\arg \c MCHR_ERROR_ADD_SENSOR: Une erreur s'est produite pendant l'ajout du capteur ?la liste : probléme de communication avec le capteur.
			\arg \c MCHR_ERROR_NAME_ALREADY_EXISTS: un capteur du même nom existe déja dans la liste.
		\else
			\param [in] LPCSTR SensorName : Name of the housing to connect to.
			\param [in] enChrType SensorType : Type of the housing.
...
/*!
	...
	\if FR
		\brief Fonction permettant d'etablir une connexion RS232 avec un capteur et l'ajjoute ?la liste des capteurs.
	\else
...
/*!
	...
	\details
		\if FR
			\param [in] MCHR_ID  SensorID : ID du capteur ?deconnecter.
			\return short : un code d'erreur MCHR_ERROR
		\else
...
/*!
	...
	\if FR
		...
		\brief Avant d appeller cette fonction il est recommand?d'appeller MCHR_GetMaxPenNumber() qui informe sur le nombre disponible  dans la table de calibration pour ce type de capteur, afin de d iniatialiser correctement le second argument.
		\warning Voir la fonction MCHR_GetPenListEx pour utiliser le parametre pFullScaleList dans un format DWORD (?partir de la DLL_CHR V3.2.0.00)!
	\else
...
/*!
	...
	\if FR
		...
		\warning Voir la fonction MCHR_GetFullScaleEx pour utiliser le parametre pFullScale dans un format DWORD (?partir de la DLL_CHR V3.2.0.00)!
	\else
...
/*!
	...
	\if FR
		...
		\warning Voir la fonction MCHR_GetFullScaleInterferometricEx pour utiliser le parametre pFullScale dans un format DWORD (?partir de la DLL_CHR V3.2.0.00)!
	\else
...
/*!
	...
	\if FR
		\brief OBSOLETE : Fonction de configuration des donnees ?transmettre sur la liaison serie.
	\else
...
/*!
	...
	\if FR
		\brief Fonction de configuration des donnees ?transmettre sur la liaison digitale.
	\else
...
/*!
	...
	\if FR
		...
		\warning Voir la fonction MCHR_GetAnalogOutputEx pour utiliser les parametres ZeroValue et MaxValue dans un format DWORD (?partir de la DLL_CHR V3.2.0.00)!
	\else
	...
	\details
		\if FR
			\param [in] MCHR_ID  SensorID : ID du coffret.
			\param [out] WORD AnaIndex		: Numero de la sortie analogique ?configurer
...
/*!
	...
	\if FR
		...
		\warning Voir la fonction MCHR_SetAnalogOutputEx pour utiliser les parametres ZeroValue et MaxValue dans un format DWORD (?partir de la DLL_CHR V3.2.0.00)!
	\else
...
/*!
	...
	\details
		\if FR
			\param [in] MCHR_ID  SensorID : ID du coffret.
			\param [in] WORD Average : entier indiquant le nombre d’acquisition successives ?moyenner pour obtenir le dark de reference. Si egal ?0 on utilise les valeurs par defaut du chr.
...
/*!
	...
	\details
		\if FR
			\param [in] MCHR_ID  SensorID : ID du coffret.
			\param [out] PWORD pwFirstUsablePixel : retourne le numero du premier pixel utilis?pour l'acquisition du spectre
...
/*!
	...
	\details
		\if FR
			...
			\arg \c  MCHR_ERROR_WRITE_CONFIG_FILE: Erreur pendant l'essai d'écriture de la configuration dans le fichier spécifi?
		\else
			\param [in] MCHR_ID  SensorID : ID of the device.
...
/*!
	...
	\if FR
		...
		\warning Voir la fonction MCHR_GetOpticalPenEx pour utiliser le parametre pFullScale dans un format DWORD (?partir de la DLL_CHR V3.2.0.00)!
	\else
...
/*!
	...
	\if FR
		...
		\warning Voir la fonction MCHR_SetOpticalPenEx pour utiliser le parametre pFullScale dans un format DWORD (?partir de la DLL_CHR V3.2.0.00)!
	\else
...
/*!
	...
	\details
		\if FR
			\param [in] MCHR_ID  SensorID : ID du coffret.
			\param [in] WORD wIndexFileIndice : pointeur sur le nom de fichier d'indice ?selectionner
...
/*!
	...
	\if FR
		...
		\warning Appeler GetNbrPixels pour connaître la taille du buffer ?passer en paramètres 
	\else
...
/*!
	...
	\if FR
		\brief Fonction ?utiliser pour recuperer le fichier d'indice de refraction
		\warning Appeler GetNbrPixels pour connaître la taille du buffer ?passer en paramètres
	\else
...
/*!
	...
	\details
		\if FR
			\param [in] MCHR_ID  SensorID : ID du coffret.
			\param [in] char *FirmwareFile : Nom du fichier contenant le firmware (.exe) ?telecharger   = NULL si on ne souhaite pas telecharger le firmware (.exe)
			\param [in] char *AdditionalFile : Nom du fichier complementaire contenant le Menu ?telecharger pour CHR 450. Nom du fichier complementaire 'stil.bin' ?telecharger pour un CHR150. = NULL si on ne souhaite pas telecharger le menu complementaire
...
/*!
	...
	\details
		\if FR
			...
			\param [out] UINT uiMessID : ID du message qui est envoye ?la fenêtre
			\return short : un code d'erreur MCHR_ERROR.
		\else
			\param [in] MCHR_ID  SensorID : ID of the device.
...
/*!
	...
	\if FR
		\brief Acquiere le black signal ?toutes fréquences, et retourne la fréquence minimun.
	\else
...
/*!
	...
	\if FR
		...
		\warning Voir la fonction MCHR_GetMaxCalibrationFileValueEx pour utiliser le parametre pFullScale dans un format DWORD (?partir de la DLL_CHR V3.2.0.00)!
	\else
...
/*!
	...
	\details
		\if FR
			\param [in] MCHR_ID  SensorID : ID du coffret.
			\param [in] char *ColorRefFile : Nom du fichier contenant les paramètres de reference de couleurs ?telecharger
...
/*!
	...
	\details
		\if FR
			\param [in] MCHR_ID  SensorID : ID du coffret.
			\param [in] char *RefSpectrumFile :  Nom du fichier contenant le spectre de reference ?telecharger
...
/*!
	...
	\details
		\if FR
			\param [in] MCHR_ID  SensorID : ID du coffret.
			\param [out] long *pMinFreq : Tableau d'entiers allou?par l'appelant et rempli par la DLL avec les frequences minimum permises sur chaque canal
			...
			\return short : un code d'erreur MCHR_ERROR. la fonction retourne une erreur si elle est appel?pour un seul canal du capteur.
		\else
			\param [in] MCHR_ID  SensorID : ID of the device.
